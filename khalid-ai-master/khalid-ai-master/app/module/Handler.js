...
const handler = (data) => {
  ...
  client.message(message)
    .then((intentData) => {
      ...
      } else if (intentData.entities.iot_things[0].value) {
        let namaThing = intentData.entities.iot_things[0].value;
        let state = intentData.entities.on_off[0].value === 'on' ? true : false;
        newHandleIoTAction(senderID, namaThing, state);
      }
    });
};

const newHandleIoTAction = async (senderID, thingName, state) => {
  const thingRef = firebase.database().ref(`/Things/`);
  thingRef.once('value', snapshot => {
    let newThingList = snapshot.val().map((thing) => {
    ...
    thingRef.update(newThingList).then(() => {
      let aksi = ''
      if (state) {
        aksi = 'on'
      } else {
      ...
      messenger.sendTextMessage(senderID, `${thingName} is ${aksi}`);
    })
  })
}
